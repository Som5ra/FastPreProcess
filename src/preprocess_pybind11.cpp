...
template<typename in_t, typename out_t> py::array_t<out_t> hwc_to_chw(py::array_t<in_t> input, bool flip_rb){
    ...
    _hwc_to_chw((in_t *)info.ptr, input_shape, result, flip_rb);
...
template<typename in_t, typename out_t> py::array_t<out_t> chw_channel_normalize(py::array_t<in_t> input, std::vector<out_t> mean, std::vector<out_t> std, bool flip_rb){
    ...
    out_t *result = new out_t[info.shape[0] * info.shape[1] * info.shape[2]];
    _chw_channel_normalize((in_t *)info.ptr, input_shape, mean.data(), std.data(), result);
...
template<typename in_t, typename out_t> py::array_t<out_t> hwc_to_chw_normalize(py::array_t<in_t> input, std::vector<out_t> mean, std::vector<out_t> std, bool flip_rb){
    ...
    out_t *result = new out_t[info.shape[0] * info.shape[1] * info.shape[2]];
    _hwc_to_chw_normalize((in_t *)info.ptr, input_shape, mean.data(), std.data(), result, flip_rb);
...
template<typename in_t, typename out_t> py::array_t<out_t> hwc_to_chw_normalize_batched(py::array_t<in_t> input, std::vector<out_t> mean, std::vector<out_t> std, bool flip_rb){
    auto info = input.request();
    size_t input_shape[] = {info.shape[0], info.shape[1], info.shape[2], info.shape[3]};

    out_t *result = new out_t[info.shape[0] * info.shape[1] * info.shape[2] * info.shape[3]];
    _hwc_to_chw_normalize_batched((in_t *)info.ptr, input_shape, mean.data(), std.data(), result, flip_rb);

    py::capsule free_when_done(result, [](void *f) {
        delete (out_t*)f;
    });
    return py::array_t<out_t>(
        {info.shape[0], info.shape[3], info.shape[1], info.shape[2]},
        result,
        free_when_done
    );
}
...
PYBIND11_MODULE(fastpreprocess, m) {
    ...
    m.def("hwc_to_chw_normalize_batched", &hwc_to_chw_normalize_batched<uint8_t, float_t>, py::arg("input"), py::arg("mean"), py::arg("std"), py::arg("flip_rb") = false, py::return_value_policy::take_ownership);
}